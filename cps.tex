...
Ein weiteres wichtiges Konzept ist die Nonrepudiation. % de: Revisionssicherheit
...
Hierbei stellen A1 und A3 Täuschungsangriffe\todo{Deception?} dar, A4 und A2 \gls{dos} und A5 einen physischen Angriff.
...
\subsection{Täuschungsangriff (Deception)\todo{Deception vs Täuschung}}\label{subsec:tauschung} % Deception
...
Manipulationen nutzen,\todo{Komma richtig?} hier also die Zustandsberechnung und deren Datenquelle eines \cps aus.
...
Physische Angriffe können nicht algorithmisch verhindert werden, sodass meist durch physische Zugriffseinschränkungen, wie etwa Wachpersonal oder Kameras Sicherheit geschaffen werden kann.\todo{missing cite}
...
Für \glspl{ics} schlägt das \gls{nist} in \cite{SPL+15} beispielsweise Sicherheitskonzepte vor wie den Schutz von physischen Standorten, Zugangskontrollen oder Überwachung von Mitarbeitern und Ressourcen.
Für Zähler in Stromnetzen gibt das \gls{nist} in \cite{PB14} vor, dass diese gegen Modifikationen kryptografische Module nutzen, versiegelt und geschützt sein müssen.
...
Klassische Angriffserkennungssysteme beobachten üblicherweise Netzwerke oder Cybersysteme auf Angriffe.
...
Abgesehen von menschlicher Interaktion sollten, gerade in Echtzeitsystemen, automatische Prozesse eine Wiederherstellung ermöglichen.
...
Dieses Verfahren findet beispielsweise Anwendung bei manipulierten Sensoren und bietet einen Vorteil gegenüber Redundanz und Diversität, da dieses auch funktioniert, wenn alle Sensoren kompromittiert sind.
