...
\begin{document}
    ...
    \begin{abstract}
        ...
        Deshalb soll in dieser Arbeit ein Überblick über potenzielle Angreifergruppen und Angriffsszenarien gegeben werden, um adäquate Gegenmaßnahmen vorzustellen.
        Gegenmaßnahmen können dabei sowohl proaktiv als auch reaktiven eingesetzt werden, um \cps zu schützen.

        Zuletzt soll ein Fazit zur aktuellen Sicherheitssituation getroffen werden.
    ...
    Einsatzorte für diese Systeme sind beispielsweise intelligente Stromnetze, symbiotische Sensornetzwerke für die Agrarwirtschaft und Katastrophenabwehr, medizinische oder assistierende Geräte, intelligente Verkehrssteuerung, intelligente Gebäude~\cite{RLS+10}, \gls{scada}, Autos und \glspl{ics} sein~\cite{HLL+17}.
    ...
    Ersteres stellt einen im Jahr 2010 stattgefundenen, umfassenden Angriff gegen \gls{scada} Systeme in einem Nuklearkraftwerk dar~\cite{Langner11}.
    Das Vorgehen bei diesem Angriff war zu diesem Zeitpunkt neu, da es von dem eines Cyberangriffs abwich~\cite{Langner11}.
    ...
    Dieses Problem wird verstärkt, indem in vielen monolithisch Sensoren heutzutage Mikrocontroller eingesetzt werden, wodurch es möglich ist Sensortests zu bestehen und im Produktionsbetrieb allerdings nicht wie erwartet zu funktionieren.~\cite{KLG15}\todo{Beispiel: VW}
   ...
   Abseits des TCP/IP Stacks bietet die \acrlong{dtls} eine Sicherung von UDP und das und das \acrlong{llcp} eine sicherere Kommunikation über \acrlong{nfc} an~\cite{SPB+16}.
    ...
    Das implementieren von Gegenmaßnahmen liegt momentan in der Verantwortung der einzelnen Herstellern von \cps.
    Mit dem Aufkommen von Konsumgüter für das IoT wird es immer wichtiger, dass Hersteller ein Bewusstsein für Confidentiality, Integrity, Availability, Authenticity, Veracity und Non-repudiation entwickeln.
    Aber nicht nur in neuen Systemen ist dieses von großer Wichtigkeit.
    Auch bereits eingesetzten SCADA Systemen müssen im Hinblick auf diese Eigenschaften überarbeitet der eventuell sogar komplett ausgetauscht werden.
    ...
    Dies belegt ein massives Sicherheitsproblem in dem Bereich der \cps.

    Angriffe wie beispielsweise Stuxnet (siehe \nameref{sec:intro}) zu verhindern liegt auch im Interesse der Europäischen Union.
    In der EU finden derzeit Diskussionen zu automatisierten und intelligenten Autos statt.
    Zwar gibt es schon Regelungen zur Abnahme von Autos und Datenschutzgesetze, aber es fehlt noch ein vollständiger rechtlicher Rahmen für intelligente und selbstfahrende Autos~\cite{EuropeanCommission2018}.
    ...
    Der Fokus in der Sicherheit von Autos liegt allerdings bei Cyber-Systemen.
    So wird in~\cite{EuropeanCommission2018} weniger die physische Seite von \cps in Autos beleuchtet.
    ...
    Besonders bei der Koppelung mit physischen Prozessen, wie im Beispiel der intelligenten Autos, wird Sicherheit immer wichtiger.



    %zur Sicherheit von IoT Geräten~\cite{Pinto2018}
